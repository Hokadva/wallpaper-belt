"""A module with an engine for managing desktop wallpapers"""
import ctypes
import os
import tempfile
import time
...
class WallpaperBelt:
    """Desktop wallpaper manage engine"""
    def __init__(self):
        """Setting the basic settings of the engine"""
    ...
    def set_wallpaper(self, path):
        """The function responsible for the logic of changing the wallpaper"""
    ...
    def stop(self):
        """
        Stop all wallpaper processes and clean up temporary files.
        Stops the visibility timer, media playback, GIF animations,
        and removes temporary wallpaper files from disk.
        """
        self.visibility_timer.stop()
        self._stop_all()

        if (self.current_wallpaper_path and
           os.path.exists(self.current_wallpaper_path)):
            try:
    ...
    def _stop_all(self):
        """
        Stop all media playback and GIF animations.
        Resets the internal state for media player, GIF timer,
        and the last GIF update timestamp.
        """
    ...
    def _set_pixmap(self, pixmap, file_path=None):
        ...
        config = config_manager.load_config()
        settings = (
            config_manager.get_file_settings(config, file_path)
            if file_path else {})
    ...
    def _update_gif_frame(self):
        ...
        if (self.gif_movie and
           self.gif_movie.state() == QMovie.MovieState.Running):
            pixmap = self.gif_movie.currentPixmap()
            if not pixmap.isNull():
                self._apply_frame_fast(pixmap)

    def _apply_frame_fast(self, pixmap):
        """Fast frame application without complex processing"""
        screen = QGuiApplication.primaryScreen()
        screen_size = screen.size()

        config = config_manager.load_config()
        settings = config_manager.get_file_settings(
            config, self.current_file_path)
    ...
    def _process_video_frame(self, frame):
        """The video footage is also with trottling"""
        if not self.desktop_visible or not frame.isValid():
            return

        now = time.time()
        if now - self._last_gif_update < 0.1:
            return
        self._last_gif_update = now
    ...
    def _crop_with_focus(self, pixmap, target_size, focus_point):
        """
        Crop a pixmap to the target size based on the specified focus point.
        The focus point determines which part of the image is preserved
        when cropping (e.g., "top_left", "center", "bottom_right").
        Returns:
    ...
    def _apply_wallpaper(self, image_path):
        """
        Apply the given image as the desktop wallpaper.
        Sets the wallpaper style to Fill (10) and updates the system
        parameters to change the desktop background immediately.
        Args: image_path (str): Absolute path to the image file.
        """
        try:
    ...
    def _check_desktop_visibility(self):
        """
        Check whether the desktop is currently visible.
        Determines if a fullscreen application is in the foreground
        and pauses or resumes GIF animations and video playback
        accordingly to save system resources.
        """
        try:
            ...
            if fg:
                ...
                if was_visible and not self.desktop_visible:
                    ...
                    if (self.gif_movie and self.gif_movie.state() ==
